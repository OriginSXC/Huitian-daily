...
function makeTasks() {
  ...
  if (report.isAutoPush !== false && report.cron) {
    tasks.push({ cron: report.cron, name: "回天日报-日报推送",     fnc: "dailyReport" })
  }
  const moyu = cfg("moyu")
  if (moyu.isAutoPush !== false && moyu.cron) {
    tasks.push({ cron: moyu.cron,   name: "回天日报-摸鱼日历推送", fnc: "dailyMoyu" })
  }
  const bgm = cfg("bangumi")
  if (bgm.isAutoPush !== false && bgm.cron) {
    tasks.push({ cron: bgm.cron,    name: "回天日报-今日番剧推送", fnc: "dailyBangumi" })
  }
  const cleanup = cfg("cleanup")
  if (cleanup.enable !== false && cleanup.cron) {
    tasks.push({ cron: cleanup.cron, name: "回天日报-定期清缓存",  fnc: "scheduledClean" })
  }
  return tasks
}
